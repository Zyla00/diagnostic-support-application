...
# Czyszczenie: tylko etykiety muszą być
df["Opis_przypadku"] = df["Opis_przypadku"].fillna("")
df["Alergie"] = df["Alergie"].fillna("")
df["Choroby_przewlekłe"] = df["Choroby_przewlekłe"].fillna("")
df = df.dropna(subset=["Jednostka_medyczna"])
df["tekst"] = df["Opis_przypadku"] + " " + df["Alergie"] + " " + df["Choroby_przewlekłe"]

# Label encoding
label_encoder = LabelEncoder()
df["label"] = label_encoder.fit_transform(df["Jednostka_medyczna"])
...
embeddings = [get_herbert_embedding(t) for t in tqdm(df["tekst"])]
...
clf = XGBClassifier(
    n_estimators=300,objective="multi:softprob", num_class=len(label_encoder.classes_), eval_metric="mlogloss", n_jobs=-1)
clf.fit(
    X_train, y_train,
...
print("Zapisano metryki i model.")
